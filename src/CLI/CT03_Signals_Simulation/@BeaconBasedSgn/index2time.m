...
%   Detailed explanation goes here
    ...
    if strcmp(index_traj,"end")
        if inlevel
            index_traj = length(iBBS.BeaconBasedSgnLevels);
        else
            index_traj = length(iBBS.BeaconBasedSgnLevels) - 1;
        end
    end
    
    if strcmp(index_node,"end")
        if inlevel
            index_node = iBBS.BeaconBasedSgnLevels(index_traj).len;
        else
    ...
    if inlevel
       Tcurrent =  iBBS.BeaconBasedSgnLevels(index_traj).ms(index_node).t;
    else
...
function Tbefore = sum_interlevel(iBBS,index_traj,inlevel)
    ...
    for index = 1:index_traj-1
        Tbefore = Tbefore + iBBS.BeaconBasedSgnLevels(index).ms(end).t + iBBS.dt;
        Tbefore = Tbefore + iBBS.intersignal{index}(end).t + iBBS.dt;
    end

    if ~inlevel 
        Tbefore = Tbefore + iBBS.BeaconBasedSgnLevels(index_traj).ms(end).t + iBBS.dt;
    end
end
