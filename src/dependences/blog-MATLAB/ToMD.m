function ToMD(all,CT,T)

 
[~ , answer] = system('find examples/* -name "T0*.m"');
...
for index = 1:length(answer)
    ...
    publishreadme(name_with_slash,['examples/',path],false);
    %publishjekyllpost([name_with_slash,'.m'],['examples/',path],false);
    clear;
    load('ToMD_workspace');
    % borramos html 
    delete(['examples/',path,'/',replace(name,' ','_'),'.html']);
